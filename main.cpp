...
void DrawTextC(const char *text, Vector2 center, float fontSize, Color color) {
  DrawTextEx(
      font, text,
      {center.x - MeasureText(text, fontSize) * 0.5f, center.y - fontSize * 0.5f},
      fontSize, 0, color);
};
...
static const int mapH = 10, mapSize = 50;
static const int mapVol = mapSize * mapSize * mapSize;
static const int texR = 16;

static const bool trees = false;
...
void genMap() {
  ...
  if (trees) {
    ...
    printf("yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy\n");
...
void init() {
  ...
  camera.position = (Vector3){mapSize * 0.5f, 20.0f, mapSize * 0.5f};

...
int main(void) {
  ...
  const char *names[typeC]{"T.N.T.", "Sand",  "Stone", "Dirt", "Grass",
                           "White",  "Black", "Red",   "Glass"};
  ...
  while (!WindowShouldClose()) {
    ...
    for (int i = 0; i < maxBlocks; i++) {
      ...
      float dist = (abs(camera.position.x - blocks[i].pos.x) +
    ...
    DrawTextEx(font, TextFormat("fps: %i", GetFPS()), {10, 10}, 20, 0,
               textColor);
    ...
    DrawTextEx(font, TextFormat("rot: %.1f; %.1f", rotation, h), {10, 70}, 20,
               0, textColor);

    DrawTextEx(font, TextFormat("p: %i", p), {10, 100}, 20, 0, textColor);

    DrawTextEx(font, names[cType], {10, 130}, 20, 0, colors[cType]);
    DrawTextEx(font, TextFormat("blocks: %i", cBlocks), {10, 160}, 20, 0,
               textColor);

    DrawTextEx(font,
               TextFormat("looking at: %i, %i, %i", (int)lookAt.x,
    ...
    rotation +=
        GetMouseDelta().x /
        10; //(IsKeyDown(KEY_RIGHT) - IsKeyDown(KEY_LEFT))*camera.fovy*dt;
    if (rotation > 360)
      rotation -= 360;
    if (rotation < 0)
      rotation += 360;
    ...
    // view up & down
    h += GetMouseDelta().y /
         10; //(IsKeyDown(KEY_UP) - IsKeyDown(KEY_DOWN))*dt*camera.fovy/3;
    ...
    if (
        (p && IsMouseButtonDown(MOUSE_BUTTON_RIGHT)) ||
        (!p && IsMouseButtonPressed(MOUSE_BUTTON_RIGHT)) &&
        lookAt.y > -1.0f
    ) {
      if (isEmpty(lookAt)) {
        for (int i = 0; i < maxBlocks; i++) {
        ...
        for (int i = 0; i < maxBlocks; i++) {
          if (blocks[i].type != 0 || !vec3eq(lookAt, blocks[i].pos))
            continue;
    ...
    } else if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) * p ||
               IsMouseButtonPressed(MOUSE_BUTTON_LEFT) * (!p)) {
        ...
        for (int k = 0; k < 2; k++) {
          ...
          for (int x = -s / 2; x < s / 2 + 1; x++) {
            for (int z = -s / 2; z < s / 2 + 1; z++) {
              if (abs(z) < s / 2 && abs(x) < s / 2 || (y == 6 && k == 1))
                continue;
              Vector3 pos{lookAt.x + (float)x, lookAt.y + (float)y,
                          lookAt.z + (float)z};
              if (!isEmpty(pos))
                continue;

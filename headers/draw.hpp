...
static void initTextures(const char* tpName) {
    std::string tfName;
    std::string cTp;

    for(int i = 0; i < texC; i++) {
        cTp = (FileExists(TextFormat("texturePacks/%s/%s.png", tpName, tNames[i])))
            ? tpName
            : "default";

        // if(!FileExists(TextFormat("texturePacks/%s/%s.png", tpName, tNames[i].c_str()))) {
        //     printf("%s not exsits\n", tNames[i].c_str());
        // }

        UnloadTexture(textures[i]);
        textures[i] = LoadTexture(TextFormat("texturePacks/%s/%s.png", cTp.c_str(), tNames[i]));
    }

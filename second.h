...
static const int W = 1920; // window width
static const int H = 1080; // window height
static const int CX = W / 2, CY = H / 2;
static const int maxBlocks = 3000;

static const int reloadDist = 100;

static Vector3 camTarget = Vector3Zero();
static float dt, rotation, a;
static float dx, dy;
static float h = 0;
static float rad = 4.0f;
...
void reloadNear(Vector3 pos, float maxD) {
  for (int i = 0; i < maxBlocks; i++) {
    Block *b = &blocks[i];
    if (b->type != -1 && abs(b->pos.x - pos.x) < maxD &&
        abs(b->pos.y - pos.y) < maxD && abs(b->pos.z - pos.z) < maxD) {
...
void init() {
  ...
  Image img =
      GenImageChecked(3, 3, 1, 1, {194, 178, 128, 255}, {184, 169, 121, 255});
